...
public class AnnoScolasticoService {
	...
	public ExtDirectStoreResult<AnnoScolastico> sel(ExtDirectStoreReadRequest request, Locale locale) throws NamingException, SQLException {
		List<AnnoScolastico> ans = new LinkedList<>();
		Integer institute = (Integer)request.getParams().get("institute");
		
		Connection conn = null;
		ResultSet rs = null;
	    CallableStatement by_institute = null;
	    try{
		    conn = DataHelper.myConnection();
		    conn.setAutoCommit(false);
			// crea prepared statement
			by_institute = conn.prepareCall("{ ? = call anni_scolastici_list(?) }");
			// compila prepare statement
			by_institute.registerOutParameter(1, Types.OTHER);
	
			by_institute.setLong(2, institute);
	
			by_institute.execute();
			rs = (ResultSet) by_institute.getObject(1);
			// ottenuto il result set
			while (rs.next()) {
				ans.add(new AnnoScolastico(rs.getLong(1),
						rs.getLong(2),
						rs.getLong(3),
						rs.getString(4),
						rs.getDate(5), 
