...
import org.scuola247.desktop.beans.Materia;
...
public class MateriaService {
	...
	public ExtDirectStoreResult<Materia> list(ExtDirectStoreReadRequest request, Locale locale) throws NamingException, SQLException {
	    ...
	    try{
			...
			by_institute = conn.prepareCall("{ ? = call materie_list(?) }");
			...
			while (rs.next()) {
				ans.add(new Materia(rs.getLong(1),rs.getLong(2),rs.getLong(3),rs.getString(4)));
			} 
	    }
